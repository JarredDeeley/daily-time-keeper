...
mod prelude {
    ...
    pub use eframe::egui::{CentralPanel, Context, ScrollArea, Button, TopBottomPanel, TextEdit, Key, Visuals, Ui};
    pub use eframe::{App, egui, Frame, NativeOptions, run_native};
...
fn render_tags(ui: &mut Ui, world: &mut World) {
    let mut tags = world.query::<(Entity, &Tag)>();

    for (_, tag) in tags.iter_mut(world) {
        ui.horizontal(|ui| {
            let button_text;
            if tag.is_active_segment {
                button_text = "Stop";
